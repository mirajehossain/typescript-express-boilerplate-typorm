  ...
  describe('POST /login', () => {
	...
	it('should not able to login with correct email & wrong password', (done) => {
		...
		.end((err, res) => {
			...
			res.body.message.trim().should.equal('password is incorrect');
			done();
		});
	});
	...
	it('should not able to login with wrong email & correct password', (done) => {
		...
		.end((err, res) => {
			...
			res.body.success.should.equal(false);
			res.body.should.have.property('message');
			res.body.message.trim().should.equal('email not found');
			done();
		});
	});
  ...
  describe('PUT /users/profile', () => {
	...
	describe('Password change', () => {
		...
		chai.request(app)
			...
			res.should.have.status(401);
			res.body.success.should.equal(false);
			res.body.message.trim().should.equal('incorrectPassword');
			done();
			});
		});
